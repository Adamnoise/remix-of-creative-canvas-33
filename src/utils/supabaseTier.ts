...
/** One match row from `winmix_matches`, joined with team display names. */
const CloudMatchSchema = z.array(
  z.object({
    ...
    home_display: z.string(),
    away_display: z.string(),
...
export async function fetchCloudSeasonList(league?: League): Promise<CloudSeasonMeta[]> {
  ...
  const raw = await restGet(
    `${filter}select=id,league,season_index,name,file_name,content_hash,match_count,order_mode,created_at&order=league,season_index.asc`
  );
...
export async function fetchCloudSeasonData(meta: CloudSeasonMeta): Promise<CloudSeasonDownload> {
  const raw = await restGet(
    `winmix_matches?season_id=eq.${encodeURIComponent(meta.id)}&select=match_no,match_date_raw,kickoff_iso,home_display,away_display,ht_home_score,ht_away_score,home_score,away_score&order=match_no.asc`
  );
  ...
  const lines = parsed.data.map((m) => {
    ...
    return [date, m.home_display, m.away_display, htHome, htAway, m.home_score, m.away_score].join(',');
  });
  return { meta, csvText: [header, ...lines].join('\n') };
}
