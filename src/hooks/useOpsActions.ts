...
import { fetchCloudSeasonList, fetchCloudSeasonData, ingestSeasonsToCloud, type CloudSeasonMeta, type IngestResult } from '../utils/supabaseTier';
  ...
  const downloadFromCloud = useCallback(async (league: League) => {
    ...
    try {
      ...
      const downloads = await Promise.all(
        seasonList.map((meta) => fetchCloudSeasonData(meta))
      );
